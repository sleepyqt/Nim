...
proc abi_init_func_state*(module: BModule): FuncState
proc abi_coerce1*(module: BModule; typ: PType): TypeRef
proc abi_coerce2*(module: BModule; typ: PType): array[2, TypeRef]
proc abi_classify*(module: BModule; typ: PType; state: var FuncState): ParamClass
proc abi_classify_return*(module: BModule; typ: PType; state: FuncState): ParamClass
...
proc abi_classify_amd64_systemv(module: BModule; typ: PType; state: var FuncState): ParamClass =
  if typ.kind in {tyBool}:
    ...
    if size <= 8:
      let coerced = abi_coerce1(module, typ)
      ...
      let coerced = abi_coerce2(module, typ)
...
proc abi_classify_amd64_windows(module: BModule; typ: PType; state: var FuncState): ParamClass =
  if typ.kind in {tyArray, tyObject, tyTuple}:
    result = ParamClass.Indirect
  else:
    result = ParamClass.Direct
